...
class MLDDatasetRL(Dataset):
    ...
    def obtain_reward(self, sample):
        ...
        Lev = [self.tag_seq.get_label(seq_t, output_query, return_length=True)[1] for seq_t in trajectory[:-2]] + [0, 0]
        Lev = np.array(Lev)
    ...
    def load_edit_gen(self):     # edit can do with gen
        ...
        for sample in tqdm(self.samples):
            ...
            if sample.get('trajectory', None) is None:
                sample['trajectory'] = [input_query, output_query]
            sample['trajectory'].extend([output_query])
